...
def test_route_file_does_not_overwrite_existing_file(router_paths):
    ...
    existing_file = os.path.join(category_directory, "mail_0001.txt")
    with open(existing_file, "w", encoding = "utf-8") as file:
        file.write("Старый файл")

    result = route_file(file_path, output_directory, "software_issues")
    new_destination_path = os.path.join(category_directory, "mail_0001_1.txt")
    assert result["success"] is True
...
def test_get_unique_path_with_several_conflicts(tmp_path):
    ...
    second_path = os.path.join(str(tmp_path), "mail_0001_1.txt")
